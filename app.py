...
st.write("In order to make a decision on which model to deploy data exploration is performed. The data given is a time series, therefore season decomposition is performed.")
...
st.write("The model used for prediction is a simple stacking up of linear layers. Since the amount of data is small we have to be careful about the overfitting so in between the linear layers, regularisation layers have been added."
         " In all neural network activation is necessary so for activating the netwroks, rectified linear unit are stacked up within linear layer.")


st.header("Training")
st.write("The training is done by making custom dataset of the inputs of the neural network which are the features."
         " The mean squared error loss is calculated between the sales given in the dataset and the output of the neural netwrok."
         " The model works best when trained with 25 epochs but feel free to experiement with different number of epochs. Althogh, the model can"
...
st.write("Hidden units is something that can be toyed around with. Model gives best output with hidden units as 100.")
...
if st.button('Run Model'):
    ...
    st.header("Testing")
    st.write("The testing is done by dividing the dataset into two parts. The training contains 90 percent of the dataset and 10 percent for"
             ...
             " This inference shows that the model hasn't overfitted and the predicted values would be close to the real ones. Unfortunately, we"
             " do not have much data because of which sometimes testing loss is less than training loss. Therefore, I feel it might not be the best way to determine validity.")
    plt.figure()
    ...
    st.write("Test Batch Losses:", last_5_losses)
    st.write("Total test Loss:", test_loss)
    st.pyplot(figure)
    ...
    st.write("Since, the testing loss can't be something we can be dependent on therefore I plot the values of actual and predicted data. The predicted value and actual values are sort of close to each other and we can say that this model is stable and predicts almost close to the real value.")
    ...
    st.header("Inference")
    st.write("The model that I have used currently is pretty basic regression model. There are definitely better model like LSTM , GRU for time series analysis. In our case when I tried to use that model, unfortunately I did not get decent results therefore I used regression model.")
